...
Known gap areas (see `ROADMAP.md` for the prioritized plan): a few `Get*` accessors that mirror an implemented `Set*` are still stubbed (only the ones whose `Set*` counterpart is itself unimplemented — `GetClipPlane`, `GetClipStatus`, `GetPixelShaderConstant`, `GetCurrentTexturePalette`); state blocks (`Begin/End/Apply/Capture/DeleteStateBlock`) are unimplemented while `CreateStateBlock` misleadingly returns `S_OK`.
