...
Current state (2026-08-23, end of day): 90/100 `IDirect3DDevice8` methods implemented, 10 stubs remain (listed in Phase 6). `IDirect3DSurface8` fully implements `LockRect`/`UnlockRect`/`QueryInterface`/`GetDevice` on every surface kind (Phase 2).
...
Remaining stubs, 10: `CreateAdditionalSwapChain`, `CreateVolumeTexture`, `DeletePatch`, `DrawRectPatch`/`DrawTriPatch` (RT-patch tessellation), `GetFrontBuffer`, `GetPixelShaderFunction`, `GetVertexShaderDeclaration`/`GetVertexShaderFunction`, `ProcessVertices`. The last four (`Get*ShaderDeclaration`/`Get*ShaderFunction`) need the original DX8 shader bytecode retained at `CreateVertexShader`/`CreatePixelShader` time — today only the parsed/transpiled HLSL blob is kept, not a copy of the input token stream, so returning "the original bytecode" would need new storage on `VertexShader`/`PixelShader` plus scanning the input for its `D3DVSD_END()`/`D3DSIO_END` terminator to know how much to copy. `NOT_IMPLEMENTED()` already logs file/line/function to `log.txt`, so implement the rest as specific games hit them rather than speculatively.

Texture-side long tail (lower priority, rarely called by games): `QueryInterface`, `GetDevice`, `Set/GetPrivateData`, `FreePrivateData`, `Set/GetPriority`, `GetType`, `Set/GetLOD` on `BaseTexture`. Surface-side: `GetContainer`.
