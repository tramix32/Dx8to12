...
Current state (2026-08-23, end of day): 84/100 `IDirect3DDevice8` methods implemented, 16 stubs remain (listed in Phase 6). `IDirect3DSurface8` fully implements `LockRect`/`UnlockRect`/`QueryInterface`/`GetDevice` on every surface kind (Phase 2).
...
Remaining stubs, 16 of the original 50: `CreateAdditionalSwapChain`, `CreateVolumeTexture`, `DeletePatch`, `DrawIndexedPrimitiveUP`, `DrawRectPatch`/`DrawTriPatch`, `GetFrontBuffer`, `GetGammaRamp`/`SetGammaRamp`, `GetPixelShaderFunction`, `GetRasterStatus`, `GetVertexShaderDeclaration`/`GetVertexShaderFunction`, `MultiplyTransform`, `ProcessVertices`. `NOT_IMPLEMENTED()` already logs file/line/function to `log.txt`, so implement these as specific games hit them rather than speculatively.
